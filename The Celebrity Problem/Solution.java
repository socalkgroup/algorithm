...
class GFG 
{
    ...
    static int findCelebrity(int n) 
    {
        ...
        while (st.size() > 1) 
        {
            // Step 2: Pop off top two persons from the stack, discard one person based on return status of knows(A, B).
            int a = st.pop();
            int b = st.pop();
        ...
        c = st.pop(); // celebrity candidate
  
        // Step 5 : Check if the last person is celebrity or not
        for (int i = 0; i < n; i++) 
        {
            // If 'c' know any person Or any person doesn't know 'c', return -1
